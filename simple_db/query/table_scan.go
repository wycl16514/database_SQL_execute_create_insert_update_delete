...
type TableScan struct {
	...
	rp           record_manager.RecordManagerInterface
	file_name    string
	current_slot int
}

func NewTableScan(tx *tx.Transation, table_name string, layout record_manager.LayoutInterface) *TableScan {
...
func (t *TableScan) GetRid() *record_manager.RID {
	return record_manager.NewRID(int(t.rp.Block().Number()), t.current_slot)
}
